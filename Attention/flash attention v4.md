...
正常情况下，`exp` 主要由 MUFU 计算，但 MUFU 的吞吐没有随 Tensor Core 同步增长。GPU 中的 Softmax 通常转换为 `exp2`：
...
$$
x=n+f,\qquad n\in\mathbb Z,\quad 0\le f<1
$$
...
所以 \(2^n\) 可以通过修改浮点数的指数位快速构造，不需要真的执行复杂指数运算。

### 小数部分 \(2^f\)

$$
 2^f\approx p_0+p_1f+p_2f^2+p_3f^3 
$$

使用 Horner 形式：

$$
 2^f\approx ((p_3f+p_2)f+p_1)f+p_0 
$$
...
FMA 单元和 MUFU 是不同资源，所以可以并行：

```
一部分元素 → MUFU.EX2 → 精确硬件路径
一部分元素 → FMA多项式 → 软件近似路径
```
...
#### 为什么跳过 rescale 不会改变结果？

因为分子和分母使用相同的参考值：

$$
O = \frac{\sum_j e^{s_j-r}v_j} {\sum_j e^{s_j-r}}

$$
给分子、分母同时乘任意相同系数 \(c\)：

$$
O = \frac{c\sum_j e^{s_j-r}v_j} {c\sum_j e^{s_j-r}}
$$
...
- 多项式 `exp` 是受控的数值近似；
- conditional rescaling 在实数数学上与原算法等价，只是改变何时选择新的缩放基准；
- 实际浮点计算仍可能有极小的舍入差异。
