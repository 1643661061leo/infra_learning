...
==GPU 的矩阵乘很快，等待这些数据搬运反而可能占用更多时间。因此FA1对带宽很敏感==FA1 因此被称为 **IO-aware（关注数据搬运的算法）**。这里的 IO 指 GPU 内存之间的数据移动，不是磁盘或网络。
...
K/V 块处理完立刻被下一块覆盖，完整的S、P矩阵始终没有写入HBM。
```

可能有同学会问：这样高频的往返不会造成IO压力吗？其实为了掩盖这种高频搬运的延迟，让计算单元尽量不空等，FA2/FA3采取了很多办法，这里不展开。
...
我们假设原本要存**S 和 P 这两个 N×N 的注意力矩阵**，随着N增加，会以`O(N²)`速度增长，现在只需要维护三个中间变量：
```
m: [N]           每行 running max      O(N)
l: [N]           每行 running sum      O(N)
Z: [N, d]        输出累加器            O(N d)
```
